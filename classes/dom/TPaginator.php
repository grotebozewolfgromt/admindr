...
{    
    ...
    private function processGETPOSTArray()
    {
	    	...
	    	if (array_key_exists($this->sPOSTVariableNameResultsPerPage, $_POST))
	    	{
	    		...
	    		{
	    			if (GLOBAL_COOKIES_ALLOWED) //opslaan in cookies
	    			{
    ...
    {
        return ($this->getLastPageNumber() == $this->getCurrentPage());
    }
    ...
    public function generateHTMLList($sCSSClassSelected = 'selected', $sClassNameNoLink = 'nolink', $sUrlPAGEBetweenSquareBrackets = '', $bAddPageNumbersToURLS = true)
    {
        ...
        {
            $sUrlPAGEBetweenSquareBrackets = getURLThisScript();
...
//        $objUL->addNode($objLI);
        ...
        if ($this->getCurrentPage() > $this->getFirstPageNumber(true))
        {
            ...
            $objA = new A();
            $objA->setHref($sURL);
            $objA->setText(transw('previous_results_paginator', 'Previous'));
            $objA->setTitle(transw('Previous [x] results (page [y])', '', 'x', $this->getItemCountPerPage(), 'y', $this->getPreviousPageNumber()));
        ...
        else //no link when on the first page
        {
            ...
            $objLI->addText(transw('previous_results_paginator', 'Previous'));
            $objUL->addNode($objLI);            
        }
        ...
        if (($this->getStyle() == TPaginator::STYLE_NUMBEREDPAGES))
        {
            ...
            if ($iStartOnPage > 2)
            {
                ...
                $objA->setTitle(transw('Go to the first page'));
            ...
            for ($iPageCounter = $iStartOnPage; $iPageCounter <= $iEndOnPage; $iPageCounter++)
            {
                ...
                $objA->setTitle(transw('Results [x] - [y]', '', 'x',$this->getItemRangeFrom($iPageCounter) + 1, 'y', $this->getItemRangeTo($iPageCounter) + 1));//weergave items (begint intern op 0 op het scherm op 1)
            ...
            if ($this->getPageCount(true) > $this->getMaximumNumberOfPages())
            {
                if (!$bIsLastPages)
                {
                    ...
                    $objA->setTitle(transw('Go to the last page'));//weergave items (begint intern op 0 op het scherm op 1)
        ...
        if ($bShowLinkNext)
        {
            ...
            $objA = new A();
            $objA->setHref($sURL);            
            $objA->setText(transw('next_results_paginator', 'Next'));
            $objA->setTitle(transw('Next [x] results (page [y])', '', 'x', $this->getItemCountPerPage(), 'y', $this->getNextPageNumber()));
        ...
        else //no link when on the last page
        {
            $objLI = new Li();
            $objLI->setClass($sClassNameNoLink);            
            $objLI->addText(transw('next_results_paginator', 'Next'));
            $objUL->addNode($objLI);               
        }
...
//        $objUL->addNode($objLI);
    ...
    public function generateHTMLFormWithSELECTResultsPerPage($bAddJavascriptToRefreshPage = true)
    {
        ...
        $objForm->setAction(getURLThisScript());        
        ...
        {            
            $objSelect->setOnchange('document.getElementById("'.$objForm->getNamePrefix().$objForm->getID().'").submit();');
        }
        ...
        $objText = new Text();
        $objText->setText(' '.transw('results per page'));
        $objForm->addNode($objText);
        
        //noscript toevoegen voor als er geen javascript is
        $objNoScript = new Noscript();
        $objSubmit = new InputSubmit();
        $objSubmit->setValue(transw('Change'));     
        $objNoScript->addNode($objSubmit);
        $objForm->addNode($objNoScript);
    ...
    {
        return transw('Results [x] - [y]', '', 'x',$this->getItemRangeFrom($iPageNo) + 1, 'y', $this->getItemRangeTo($iPageNo) + 1);
    }
    ...
    {
    		return transw('Results [x] - [y] (out of [z])','', 'x',$this->getItemRangeFrom($iPageNo) + 1, 'y', $this->getItemRangeTo($iPageNo) + 1, 'z', $this->getTotalItemsCount());
    }    
    ...
    public function getTextLoadMore()
    {
    		if ($this->isSecondLastPage())
    		{
    			...
    			return transw('Load last [nextresultcount] results', '', 'nextresultcount', $iItemsOnNextPage);    			
    		}
    		else
    		{ 
    			return transw('Load next [nextresultcount] results', '', 'nextresultcount', $this->getItemCountPerPage());
    		}
