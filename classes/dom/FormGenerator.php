 ...
 *  
 * This class can pull data from TModel and make an input form for most of the fields
 * 
 * 
 **********************************************************
 ...
 *       $this->objEdtUsername->addValidator($objValidator);
 *       $this->objFormLogin->add($this->objEdtUsername, '', transw('loginform_field_username', 'username')); 
 *   
...
//     }
     ...
     * @return string
     */
    public function getValueIDField(TModel $objModel)
    {
    	if ($this->getForm()->getMethod() == Form::METHOD_POST)
    		return $_POST[$objModel::getTable().TModel::FIELD_ID];
    	if ($this->getForm()->getMethod() == Form::METHOD_GET)
    		return $_GET[$objModel::getTable().TModel::FIELD_ID];    	 
    }
    ...
    private function isRecaptchaV3Valid()
    {
        ...
        if ($this->getRecaptchaV3Use() && GLOBAL_GOOGLE_RECAPTCHAV3_USE)
        {
            $sFormResponseRecapthca = $_POST['g-recaptcha-response'];
            $sURL = 'https://www.google.com/recaptcha/api/siteverify';
            $sURL = addVariableToURL($sURL, 'secret', GLOBAL_GOOGLE_RECAPTCHV3_SECRETKEY);
			...
			{
				logError(__CLASS__.': '.__FUNCTION__.': '.__LINE__, 'Recaptcha: NO Google response');
                return false;
			}
	...
	{		
		if ($this->getRecaptchaV3Use())
		{
			if (is_a($objButton, InputButton::class))
			{
				...
				$sJSFunctionNameCallback = 'onSubmitRecaptchaV3'.getSafeJavascriptFunctionName($this->objForm->getID());
				...
				$objButton->setDataSitekey(GLOBAL_GOOGLE_RECAPTCHAV3_SITEKEY);
				...
				$this->objForm->addNode($objScript);	
			...
			{
				logError(__CLASS__.':'.__FUNCTION__.':'.__LINE__, '$objButton parameter is not of type InputButton, it should be because recaptachaV3 uses a InputButton instead of a InputSubmit');
			}
     ...
     * 
     * @param TagAbstract $objDOMElement
     ...
     */
    public function add(TagAbstract $objDOMElement, $sSection = '', $sDescription = '', $bDescriptionVisible = true, $sPostDescription = '', $bMarkAsRequiredField = false)
    {
    	if ($sSection == '')
...
//        $arrTemp[FormGenerator::ARRAYKEY_FORMCONTENT_REQUIRED] = $bMarkAsRequiredField;
     ...
     * @param boolean $bMarkAsRequiredField required field
     */
    public function addArray($arrDOMElements, $sSection = '', $sDescription = '', $bDiscriptionVisible = true, $sPostDescription = '', $bMarkAsRequiredField = false)
    {
    	if ($sSection == '')
    		$sSection = FormGenerator::SECTION_DEFAULT;
    ...
    {      
		...
		//$objDivSection->addText(transw('form_input_errorstopform', 'Sorry, errors on form')); --> no text, this is done by the class caller
		if ($bErrorsOnForm) //only add to form when errors
			$objForm->addNode($objDivSection);   
	
		$objUL = new Ul();
		$objUL->setClass(FormGenerator::CSS_CLASS_FORMSECTIONERRORLIST);
		$objDivSection->addNode($objUL);
		...
		//====CSRF token errors
			...
			if ((!$bIsAntiCSRFTokenValid) && ($bSubmitted))
			{
				...
				$objDivSection->addText(transw('form_input_error_anti-csrftoken-invalid', 'Form invalid. The form most likely expired: you waited too long with submitting input. Try submitting again!'));
				$objForm->addNode($objDivSection);   
			}
        ...
        foreach ($arrSectionKeys as $sSectionName)
        {
        	...
        	if ($iCountSections > 1) 
        	{
        		...
        		$objDivHeader->addNode($objTempText);
        		$objDivSection->addNode($objDivHeader);
        	}
            	...
            	foreach ($arrSection as $arrFormLine)
            	{
            		...
            		if ($arrFormLine[FormGenerator::ARRAYKEY_FORMCONTENT_DESCRIPTIONVISIBLE])
            		{
	            		...
	            		if ($bErrorsDetectedOnLine)
                        {
	            			...
	            			$objLabel->setClass($objLabel->getClass().' '.FormGenerator::CSS_CLASS_FORMDESCRIPTIONERROR);
                        }
                                
	            		if ($iCountElementsOnLine > 0)
	            		{            			
	            			$objFirstDom = $arrElementsOnLine[0];
	            			$objLabel->setFor($objFirstDom->getNamePrefix().$objFirstDom->getID());            			
	            		} 
	                ...
	                foreach($arrElementsOnLine as $objDOMElement)
	                {
	                	...
	                	if ($bAddToLine)
	                		$objDivLine->addNode($objDOMElement);
	                }
	                ...
	                if (strlen($arrFormLine[FormGenerator::ARRAYKEY_FORMCONTENT_POSTDESCRIPTION]) > 0)
	                {
            			...
            			{            			
            				$objFirstDom = $arrElementsOnLine[0];
            				$objLblPostDescription->setFor($objFirstDom->getNamePrefix().$objFirstDom->getID());            			
            			} 	                	
	                	$objDivLine->addNode($objLblPostDescription);
	                }                       
	                ...
	                if ($iCountElementsOnLine > $iCountDeletedElementsOnLine) //als alle elementen gedelete zijn, heeft het weinig zin om de line toe te voegen
	                	$objDivSection->addNode($objDivLine);
    ...
    public function merge(FormGenerator &$objForm)
    {
    	...
    	foreach ($arrKeysThis as $sKey)
    	{
    		if (array_key_exists($sKey, $objForm))
    			error('merge(): key "'.$sKey.'" already exists in other form, therefore cannot merge without overriding values', $this);
    	}
     ...
     * populate is used by modelToForm() and formToModel() (there is some redudancy if you load records foreign tables etc)
     * 
     * @param TModel $objModel
     ...
     */
    public function populate(TModel &$objModel, $arrFields = array(), $bLoadForeignValuesFromDB = false)
    {    	
    	if (!$arrFields)
    	...
    	foreach ($arrFields as $sFieldName)
    	{
    		...
    		$bSkipField = false;
    		$sTransDescription = transw('form_'.get_class_short($this).'_'.$sFieldName, $sFieldName);
    		$sID = $sTable.$sFieldName;
    		$sName = $sID;
    	
    		if ($sFieldName == TModel::FIELD_ID) //id field is hidden
    		{
    		...
    		elseif (($objModel->getFieldForeignKeyTable($sFieldName)) && ($objModel->getFieldForeignKeyField($sFieldName)))//foreign keyed table
    		{
    			...
    			if ($bLoadForeignValuesFromDB) //load all foreign key values
    			{
    				...
    				if ($objForeignModel->getTableUseOrderField())
    					$objForeignModel->sort(TModel::FIELD_ORDER);
   					$objForeignModel->loadFromDB();
   					while ($objForeignModel->next())
   					{
   						$objOption = new Option();
   						$objOption->setValue($objForeignModel->get(TModel::FIELD_ID));
   						$objOption->setText($objForeignModel->getGuiItemName());
   						//$objOption->setSelected($objModel->get($sFieldName) === $objForeignModel->get(TModel::FIELD_ID));//selected --> kunt m nog niet selecteren record is nog niet geladen
   						$objNewDomElement->addNode($objOption);
   					}
    		...
    		elseif ($sFieldName == TModel::FIELD_CHECKOUTEXPIRES)
    			$bSkipField = true;
    		elseif ($sFieldName == TModel::FIELD_CHECKSUMENCRYPTED)
    			$bSkipField = true;
    		elseif ($sFieldName == TModel::FIELD_ORDER)
    			$bSkipField = true;
    		elseif ($sFieldName == TModel::FIELD_RECORDCHANGED)
    			$bSkipField = true;
    		elseif ($sFieldName == TModel::FIELD_RECORDCREATED)
    			$bSkipField = true;
    		elseif ($sFieldName == TModel::FIELD_RECORDHIDDEN)
    			$bSkipField = true;
    		elseif ($sFieldName == TModel::FIELD_SYS_DIRTY)
    			$bSkipField = true;
    		elseif ($sFieldName == TModel::FIELD_SYS_NEW)
    			$bSkipField = true;
    			...
    			switch($iType)
    			{
    				...
    				case CT_INTEGER64:
    					...
    					if ($objModel->getFieldNullable($sFieldName))
    						$objNewDomElement->addValidator(new Onlynumericorempty(transw('detailmodel_onlynumericorempty', 'Only numeric values or empty allowed')));
    					else
    						$objNewDomElement->addValidator(new Onlynumeric(transw('detailmodel_onlynumeric', 'Only numeric values allowed')));
    					$objNewDomElement->setID($sID);
    					$objNewDomElement->setName($sName);
    					$this->add($objNewDomElement, '', $sTransDescription);
    				...
    				case CT_CURRENCY:
    					...
    					$objNewDomElement->setClass(FormGenerator::CSS_CLASS_INPUTTEXT);
    					if ($objModel->getFieldNullable($sFieldName))
    						$objNewDomElement->addValidator(new Onlynumeric(transw('detailmodel_onlynumericorempty', 'Only numeric values or empty allowed')));
    					else
    						$objNewDomElement->addValidator(new Onlynumeric(transw('detailmodel_onlynumeric', 'Only numeric values allowed')));
    					$objNewDomElement->setID($sID);
    					$objNewDomElement->setName($sName);
    				...
    				default: //string
    					...
    					$objNewDomElement->setClass(FormGenerator::CSS_CLASS_INPUTTEXT);
    					$objNewDomElement->addValidator(new Maximumlength(transw('detailmodel_maximum_of_[x]_characters','a maximum of [x] characters', 'x', $iMaxLength), $iMaxLength));
    					$objNewDomElement->setID($sID);
    					$objNewDomElement->setName($sName);
    					$this->add($objNewDomElement, '', $sTransDescription);
     ...
     * @param array $arrFields optional: generate only html-objects for this fields, otherwise $objModel->getFieldsDefined() is assumed
	 */
    public function formToModel(TModel &$objModel, $arrFields = array())
    {    	
            $sTable = $objModel::getTable();
            if (!$arrFields)
                    $arrFields = $objModel->getFieldsDefined();
            ...
            $iID = $this->getElement($sTable.TModel::FIELD_ID)->getContentsSubmitted()->getValue();
            if (!is_numeric($iID))//preventing injection
                            $iID = TModel::FIELD_ID_VALUE_DEFAULT;
                            ...
                            switch ($iType)
                            {
                                    case CT_DATETIME:
                                            $objDateTime = new TDateTime();
                                                    $objDateTime->setDateTimeAsString($this->getElement($sHTMLID)->getContentsSubmitted()->getValue());
                                                    $objModel->set($sFieldName, $objDateTime);
                                                    break;
                                            case CT_BOOL:
                                                    $objModel->set($sFieldName, $this->getElement($sHTMLID)->getContentsSubmitted()->getValueAsBool());
                                                    break;
                                    case CT_INTEGER:
                                    case CT_INTEGER64:
                                            $objModel->set($sFieldName, $this->getElement($sHTMLID)->getContentsSubmitted()->getValueAsInt());
                                            break;
                                    case CT_DECIMAL:
                                                    $objDec = new TDecimal();
                                                    $objDec->setValueFormatted($this->getElement($sHTMLID)->getContentsSubmitted()->getValue());
                                                    $objModel->set($sFieldName, $objDec);
                                                    break;
                                    case CT_CURRENCY:
                                            $objDec = new TCurrency();
                                            $objDec->setValueFormatted($this->getElement($sHTMLID)->getContentsSubmitted()->getValue());
                                            $objModel->set($sFieldName, $objDec);
                                            break;
                                    case CT_ENUM:
                                    case CT_LONGTEXT:
                                    default://string
                                            $objModel->set($sFieldName, $this->getElement($sHTMLID)->getContentsSubmitted()->getValue());
                            }//switch
                    }//array_key_exists
            }//foreach arrFields
     ...
     * @param array $arrFields optional: generate only html-objects for this fields, otherwise $objModel->getFieldsDefined() is assumed
     */    
    public function modelToForm(TModel &$objModel, $arrFields = array())
    {
    	if (!$arrFields)
    		$arrFields = $objModel->getFieldsDefined();
    	$sTable = $objModel::getTable();
    	...
    	foreach ($arrFields as $sFieldName)
    	{
    		...
    		if (($objModel->getFieldForeignKeyTable($sFieldName)) && ($objModel->getFieldForeignKeyField($sFieldName)))//foreign keyed table --> select the correct value
    		{
    			...
    			while($objNode = $objDOMElement->getNodeNext())
    			...
    			switch($iType)
    			{
    				...
    				case CT_ENUM:
    					...
    					foreach ($arrOpts as $sOption)
    					{
    						...
    						$objOption->setText(transw('detailmodel_'.$sTable.$sFieldName.'_'.$sOption, $sOption));
    						$objOption->setSelected($objModel->get($sFieldName) === $sOption);//selected
    						$objDOMElement->addNode($objOption);
    					}
    					break;
    				case CT_LONGTEXT:
     ...
     * $objEditLocale->setValue($objEditLocale->getContentsSubmitted()->getValue());
    ...
    public function setSubmittedValuesAsValues()
    {
		...
		$this->objInputTypeHiddenAntiCSRFToken->setValue($this->objInputTypeHiddenAntiCSRFToken->getContentsSubmitted()->getValue());
     ...
     * @return string token
     */
    private function generateAndRegisterAntiCSRFToken()
    {
        $sNewToken = '';
        $sNewSalt = '';
		...
		if ($this->isAntiCSRFTokenValid())
			$_SESSION[FormGenerator::SESSIONARRAYKEY_ANTICSRFTOKEN] = array_deletevalue($_SESSION[FormGenerator::SESSIONARRAYKEY_ANTICSRFTOKEN], $this->objInputTypeHiddenAntiCSRFToken->getContentsSubmitted()->getValue());
	...
	public function isAntiCSRFTokenValid()
	{
		...
		{	
			//token doesn't exist
			if (!in_array($this->objInputTypeHiddenAntiCSRFToken->getContentsSubmitted()->getValue(), $_SESSION[FormGenerator::SESSIONARRAYKEY_ANTICSRFTOKEN]))
				return false;
		}
