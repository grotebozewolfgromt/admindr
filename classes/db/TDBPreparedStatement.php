...
abstract class TDBPreparedStatement
{
     ...
     * @return object parent object
     */
    public function setOwnerObject($objOwnerObject)
    {
    		$this->objOwnerObject = $objOwnerObject;
    }    
     ...
     */
    abstract public function executeSelect(TModel &$objModel);
     ...
     * @param TModel $objModel builder object
     * @return bool toevoegen gelukt ?
     */
    abstract public function insertModel(TModel &$objModel);
     ...
     */
    abstract public function updateModel(TModel &$objModel);
     ...
     * @return boolean
     */    
    abstract public function updateField($sTable, $sField, $sValue, $sWhereField, $sWhereValue);
...
//     }
     ...
     */
    abstract public function generateSQLSelectModel(TModel &$objModel);
     ...
     */
      ...
      * @return bool success?
      */
     abstract public function createTableFromModel(TModel &$objModel); //test with new TModel object
     ...
     */
    abstract protected function parseSQL($sSQLStatementWithPlaceholders = '');
     ...
     * WORKS ONLY IN DEVELOPMENT ENVIRONMENT! (uses GLOBAL_DEVELOPMENTENVIRONMENT)
     * 
    ...
    public function debug($sSQLStatementWithPlaceholders = '')
    {
        ...
        if (GLOBAL_DEVELOPMENTENVIRONMENT)
        {
            ...
            {
                $sGeneratedSQL = $this->parseSQL($this->getSQLPreparedStatement());
                $sSQLStatementWithPlaceholders = $this->getSQLPreparedStatement();
            }
            else
                $sGeneratedSQL = $this->parseSQL($sSQLStatementWithPlaceholders);
            ...
            if (count($this->getParameters()) != $iQuestMarksCount)
                error('the number of question marks ('.$iQuestMarksCount.') isn\'t the same as the number of bound parameters ('.count($this->getParameters()).')', $this);
