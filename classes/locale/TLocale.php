...
class TLocale
{
    ...
    public function __construct($sDefaultLocale = GLOBAL_LOCALE_DEFAULT) 
    {
        //set default
    ...
    * @return string 
    */     
    public function getLocaleFromBrowser($sDefaultLanguage = GLOBAL_LOCALE_DEFAULT)
    {
    ...
    public function setLocale($sLocale)
    {
        if ((strlen($sLocale) >= 5) || (strlen($sLocale) >= 7))
        {
            ...
            if (count($arrLocales) == 2)
            {
                ...
                error('setLocale(): the locale code must consist of 2 elements, separated with a dash (-)');
        }
        elseif ((strlen($sLocale) == 2) || (strlen($sLocale) == 3) )
        {
            ...
            error('setLocale(): locale code ('.$sLocale.') must contain 2-3 or 5-7 characters: i.e. nl or nl-NL, now its only '.strlen($sLocale).' characters.');
    }
    ...
    public function setCountryCode($sCode)
    {
        if ((strlen($sCode) == 2) || (strlen($sCode) == 3))
            ...
            error('setCountryCode(): Country code not 2 or 3 characters', $this);
    }
    ...
    public function setLanguageCode($sCode)
    {
        if ((strlen($sCode) == 2) || (strlen($sCode) == 3))
            ...
            error('setLanguageCode(): Language code not 2 or 3 characters', $this);        
    }
