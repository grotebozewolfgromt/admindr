    ...
    public function getAsHTML()
    {
        ...
        if (count($arrHeader) > 0)
        {
            ...
            for ($iColumnCounter = 0; $iColumnCounter < $iCount; $iColumnCounter++)
            {
                ...
                $objHTMLCol->addTextNode($arrHeader[$iColumnCounter]); //waarde opvragen aan de hand van de kolomnaam
                    
                $objHTMLRow->addNode($objHTMLCol);                
            }

            $objHTMLTable->getTbody()->addNode($objHTMLRow);     
        }
        ...
        foreach ($arrRowObjects as $objRow)
        {
            ...
            for ($iColumnCounter = 0; $iColumnCounter < $iCount; $iColumnCounter++)
            {
                ...
                $objHTMLCol->addTextNode($objRow->get($iColumnCounter)); //waarde opvragen aan de hand van de kolomnaam
