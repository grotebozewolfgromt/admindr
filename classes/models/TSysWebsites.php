...
class TSysWebsites extends TModel
{
	const FIELD_WEBSITENAME = 's_domein';//@todo cms2/4 compatibility, for CMS5 change to: sWebsiteName
	const FIELD_URL = 'sURL';
	const FIELD_DEFAULTLANGUAGEID = 'iDefaultLanguageID'; //id of the defaultlanguage
		
	const DEFAULT_WEBSITENAME = '[change this name to your site]';	
	...
	public function install($arrPreviousDependenciesModelClasses = null)
	{
			...
			if ($bSuccess)
			{
					...
					if (!$this->saveToDB()) { error('error saving website on install: '. $this->getWebsiteName()); }
					...
					if ($this->count() == 0) //only add when table is empty
					{
							...
							$this->newRecord();
							$this->setWebsiteName(GLOBAL_PATH_DOMAIN);
							$this->setURL(GLOBAL_PATH_WWW);

							if (!$this->saveToDB())
									error('error saving website on install');
	...
	*/
	public function defineTable()
	{
		...
		$this->setFieldForeignKeyField(TSysWebsites::FIELD_DEFAULTLANGUAGEID, TModel::FIELD_ID);
		$this->setFieldForeignKeyJoin(TSysWebsites::FIELD_DEFAULTLANGUAGEID);
		$this->setFieldForeignKeyActionOnUpdate(TSysWebsites::FIELD_DEFAULTLANGUAGEID, TModel::FOREIGNKEY_REFERENCE_CASCADE);
		$this->setFieldForeignKeyActionOnDelete(TSysWebsites::FIELD_DEFAULTLANGUAGEID, TModel::FOREIGNKEY_REFERENCE_RESTRICT);
