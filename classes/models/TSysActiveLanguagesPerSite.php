 ...
 */
class TSysActiveLanguagesPerSite extends TModel
{
       ...
       /**
		...
		if ($bSuccess)
		{
                        ...
                        if ($objLangs->loadFromDBByLocale(GLOBAL_LOCALE_DEFAULT))
                        {
                            ...
                            while ($objWebsites->next())
                            {
                                if ($objCloneActiveLangs->loadFromDBByWebsiteID($objWebsites->getID()))
                                {
                                    ...
                                    if ($objCloneActiveLangs->count() == 0) //when no languages are added for this site, create default
                                    {
                                            ...
                                            {       
                                                    $bSuccess = false;
                                                    error(__CLASS__.' '.__LINE__.' error saving language-per-site on install');                                        
                                            }
                                    }
                                }
	...
	*/
	public function defineTable()
	{
		...
		$this->setFieldForeignKeyField(TSysActiveLanguagesPerSite::FIELD_LANGUAGEID, TModel::FIELD_ID);
		$this->setFieldForeignKeyJoin(TSysActiveLanguagesPerSite::FIELD_LANGUAGEID);
		$this->setFieldForeignKeyActionOnUpdate(TSysActiveLanguagesPerSite::FIELD_LANGUAGEID, TModel::FOREIGNKEY_REFERENCE_CASCADE);
		$this->setFieldForeignKeyActionOnDelete(TSysActiveLanguagesPerSite::FIELD_LANGUAGEID, TModel::FOREIGNKEY_REFERENCE_RESTRICT);
		...
		$this->setFieldForeignKeyField(TSysActiveLanguagesPerSite::FIELD_WEBSITEID, TModel::FIELD_ID);
		$this->setFieldForeignKeyJoin(TSysActiveLanguagesPerSite::FIELD_WEBSITEID);
		$this->setFieldForeignKeyActionOnUpdate(TSysActiveLanguagesPerSite::FIELD_WEBSITEID, TModel::FOREIGNKEY_REFERENCE_CASCADE);
		$this->setFieldForeignKeyActionOnDelete(TSysActiveLanguagesPerSite::FIELD_WEBSITEID, TModel::FOREIGNKEY_REFERENCE_RESTRICT);
	...
	*/
	...
	{
		return GLOBAL_DB_TABLEPREFIX.'SysActiveLanguagesPerSite';
	}
