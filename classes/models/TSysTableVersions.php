 ...
 * 
 */
class TSysTableVersions extends TModel
{
    ...
    public function synchronizeTables()
    {
        ...
        if ($this->loadFromDB(false)) //als laden goed gegaan
        {
            ...
            {
                $iLenghtPrefix = strlen(GLOBAL_DB_TABLEPREFIX);
                $sPrefixCurrentTable = substr($sTable, 0, $iLenghtPrefix);
                if ($sPrefixCurrentTable == GLOBAL_DB_TABLEPREFIX) //only the tables with the right prefix
                {
	...
	*/
	...
	{
		return GLOBAL_DB_TABLEPREFIX.'SysTableVersions';
	}
