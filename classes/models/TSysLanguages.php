...
class TSysLanguages extends TModel
{
...
om-KE,Oromo (Kenya)';
	...
	//	$sCSV = "nl-NL,Dutch (Netherlands)
	//	en-US,English (United States)"; //-===> when you want a quick install, we can uncomment this		
		...
		if ($bSuccess)
		{
			...
			if ($this->count() == 0) //only add when table is empty
			{
				...
				foreach ($arrLines as $sLine)
				{
					...
					$this->newRecord();
					$this->set(TSysLanguages::FIELD_LOCALE, $arrColumns[0]);
					$this->set(TSysLanguages::FIELD_LANGUAGE, $arrColumns[1]);
					if ($arrColumns[0] == GLOBAL_LOCALE_DEFAULT)
					{
	...
	*/
	public function defineTable()
	{
		...
		$this->setFieldUnique(TSysLanguages::FIELD_LANGUAGE, false);
		$this->setFieldIndexed(TSysLanguages::FIELD_LANGUAGE, false);
	...
	*/
	...
	{
		return array(TSysLanguages::FIELD_LANGUAGE, TSysLanguages::FIELD_LOCALE, TSysLanguages::FIELD_ISCMSLANGUAGE, TSysLanguages::FIELD_ISSYSTEMDEFAULT, TSysLanguages::FIELD_ISVISIBLE, TModel::FIELD_ORDER);
	}
