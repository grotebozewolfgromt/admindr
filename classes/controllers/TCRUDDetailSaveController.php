 ...
 * -TModel record loading and saving is implemented by default
...
abstract class TCRUDDetailSaveController
{
    ...
    public function __construct()
    {
        ...
        if (auth($this->sModule, $this->getAuthorisationCategory(), TModuleAbstract::PERM_OP_VIEW))
        {
            ...
            if ($objCountrySettings)
            {
                $this->sDefaultDateFormat = $objCountrySettings->getCountrySetting(TCountrySettings::DATEFORMAT_DEFAULT);
                $this->sDefaultTimeFormat = $objCountrySettings->getCountrySetting(TCountrySettings::TIMEFORMAT_DEFAULT);
                $this->sDefaultDateTimeFormat = $objCountrySettings->getCountrySetting(TCountrySettings::DATETIME_FORMAT_DEFAULT);
            }            
            ...
            $this->handleSubmitted(); //-> + formToModel() is called here
            $this->modelToForm();     
    ...
    /**
     * retrieve default datetime format (PHP date() format)
     * 
     * @return string
     */    
    public function getDateTimeFormatDefault()
    {
        return $this->sDefaultDateTimeFormat;
    }
    ...
    protected function populateInternal()
    {
        ...
        $this->objCancel->setValue(transcms('form_button_close', 'close'));
//        $this->objCancel->setValue();
        $this->objCancel->setName('btnClose');    
        $this->objCancel->setClass('button_cancel');    
        $this->objCancel->setOnclick('window.location.href = \''.addVariableToURL($this->sURLThisScript, ACTION_VARIABLE_CANCEL, ACTION_VALUE_CANCEL).'\';');    
    ...
    protected function handleNewEditRecord()
    {
        ...
        if (isset($_GET[ACTION_VARIABLE_ID])) 
        {
            ...
            {        
                if ($this->objModel->getTableUseCheckout() && $this->getUseCheckinout()) 
                    $this->objModel->checkoutNowDB($_GET[ACTION_VARIABLE_ID], $this->sModule.': '.$this->getAuthorisationCategory().': database record edit screen by user: '.$objLoginController->getUsers()->getUsername()); //@todo replace by user
            }
...
//=====================================================================================    
     ...
     */
    abstract protected function formToModel();
    
    /**
     * transfer database elements to form
     */
    abstract protected function modelToForm();
