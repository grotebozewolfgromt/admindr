 ...
 */
    ...
    $arrDBColumnNamesIndices[$iColumnIndexVisible] = $arrTableColumnsShow[2];//appoint the indexes of $arrTableColumnsShow to the columns
    $arrDBColumnNamesIndices[$iColumnIndexOrder] = $arrTableColumnsShow[3];//appoint the indexes of $arrTableColumnsShow to the columns
...
<?php
//=========== FILTERS ========
//if (auth($sCurrentModule, 'filters', 'show'))
//{
                    ...
                    <tr>
                        ...
                        <?php
                            ...
                            for ($iColumnIndex = 0; $iColumnIndex < $iColumnCount; $iColumnIndex++)                  
                            {
                                ...
                                else//the fields we produce ourselves manually
                                {
                                    ...
                                    {
                                        case $iColumnIndexIcon: //icon
                                            $sIconPath = GLOBAL_PATH_WWW_CMS_IMAGES.'/icon-module32x32.png';  //default                              
                                        ...
                                        //     if (file_exists(GLOBAL_PATH_LOCAL_MODULES.DIRECTORY_SEPARATOR.$sInternalModuleName))
                                        ...
                                        case $iColumnIndexDirExists: //directory exists
                                            if (file_exists(GLOBAL_PATH_LOCAL_MODULES.DIRECTORY_SEPARATOR.$sInternalModuleName))
                                            {
                                                $sColumnValue = '<img alt="'.transcms('boolean_yes', 'yes').'" src="'.GLOBAL_PATH_WWW_CMS_IMAGES.'/icon-checked-true32x32.png">&nbsp;';
                                                $sColumnValue.= transm($sCurrentModule, 'cmsmodulelist_moduleexists_folder_yes', 'folder exists');
                                            }
                                            else
                                            {
                                                $sColumnValue = '<img alt="'.transcms('boolean_no', 'no').'" src="'.GLOBAL_PATH_WWW_CMS_IMAGES.'/icon-checked-false32x32.png">&nbsp;';
                                                $sColumnValue.= transm($sCurrentModule, 'cmsmodulelist_moduleexists_folder_no', 'folder missing');
                                            }
                                            $sColumnValue.= '<br>';
                                            if (file_exists(GLOBAL_PATH_LOCAL_MODULES.DIRECTORY_SEPARATOR.$sInternalModuleName.DIRECTORY_SEPARATOR.'index.php'))
                                            {
                                                $sColumnValue.= '<img alt="'.transcms('boolean_yes', 'yes').'" src="'.GLOBAL_PATH_WWW_CMS_IMAGES.'/icon-checked-true32x32.png">&nbsp;';
                                                $sColumnValue.= transm($sCurrentModule, 'cmsmodulelist_moduleexists_index_yes', 'index file exists');                                                
                                            }
                                            else
                                            {
                                                $sColumnValue.= '<img alt="'.transcms('boolean_no', 'no').'" src="'.GLOBAL_PATH_WWW_CMS_IMAGES.'/icon-checked-false32x32.png">&nbsp;';
                        ...
                        <td>
                            ...
                            /*
                                ...
                                {
                                    ?><a href="<?php if (isset($sURLDetailPage)) { echo $sURLDetailPage.'?'.ACTION_VARIABLE_ID.'='.$objModel->getID(); } ?>"><img src="<?php echo GLOBAL_PATH_WWW_CMS_IMAGES.DIRECTORY_SEPARATOR;?>icon-edit32x32.png"></a><?php
                                }
                                else
                                    echo '&nbsp;';   
                             * */
