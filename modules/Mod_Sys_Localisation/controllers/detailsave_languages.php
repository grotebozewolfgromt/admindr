...
class detailsave_languages extends TCRUDDetailSaveController
{
    ...
    protected function populate() 
    {
        ...
        $objValidator = new Maximumlength(transcms('form_error_maxlengthexceeded', 'The maximumlength [length] of this field is exceeded', 'length', '11'), 11);
...
//        $sTransSection = transm($this->getModule(), 'form_group_activelanguagesites', 'active language on website:');
//        
...
//        }
    ...
    {
        $this->getModel()->set(TSysLanguages::FIELD_LOCALE, $this->objEditLocale->getContentsSubmitted()->getValueAsString());
        $this->getModel()->set(TSysLanguages::FIELD_LANGUAGE, $this->objEditLanguage->getContentsSubmitted()->getValueAsString());
        $this->getModel()->set(TSysLanguages::FIELD_ISCMSLANGUAGE, $this->objChkCMSLanguage->getContentsSubmitted()->getValueAsBool());                
        $this->getModel()->set(TSysLanguages::FIELD_ISVISIBLE, $this->objChkShowInCMSSelect->getContentsSubmitted()->getValueAsBool());                
        $this->getModel()->set(TSysLanguages::FIELD_ISSYSTEMDEFAULT, $this->objChkDefaultSystem->getContentsSubmitted()->getValueAsBool());                
    }
    ...
    {  
        if ($this->getModel()->getLocale() == GLOBAL_LOCALE_DEFAULT)
        {
            $this->objEditLocale->setReadOnly(true); 
            sendMessageNotification(transm($this->getModule(), 'form_notification_defaultlocale_cantchangelocale', 'This is the default locale of the system, you can\'t change the locale itself'));
        }
        ...
        $this->objChkDefaultSystem->setChecked($this->getModel()->get(TSysLanguages::FIELD_ISSYSTEMDEFAULT));
    }
   ...
   /**
    ...
    public function getTitle()
    {
        ...
        if ($this->getModel()->getNew())   
            return transm($sCurrentModule, 'pagetitle_detailsave_languages_new', 'Create new language');
        else
            return transm($sCurrentModule, 'pagetitle_detailsave_languages_edit', 'Edit language: [language]', 'language', $this->getModel()->getLanguage());           
    }
