 ...
 */
    ...
    if ($objForm->isFormSubmitted())
        $objUseRawHtml->setChecked($objUseRawHtml->getContentsSubmitted(Form::METHOD_POST)->getValue() == 'rawhtml');
    else
        $objUseRawHtml->setChecked(true);    
    $objForm->add($objUseRawHtml, '', transm($sCurrentModule, 'csv2table_form_field_userawhtml', 'Use raw html'));
    
    
    //input raw HTML
    $objHTMLRawDirty = new Textarea();
    $objHTMLRawDirty->setAllowHTML(FILTERXSS_ALLOW_ALL);
    ...
    if ($objForm->isFormSubmitted())
        $objHTMLRawDirty->setText($objHTMLRawDirty->getContentsSubmitted()->getValue());
    else
    ...
    if ($objForm->isFormSubmitted())
        $objUseRichtextHtml->setChecked($objUseRichtextHtml->getContentsSubmitted(Form::METHOD_POST)->getValue() == 'richtexthtml');
    else
        $objUseRichtextHtml->setChecked(false);    
    $objForm->add($objUseRichtextHtml, '', transm($sCurrentModule, 'csv2table_form_field_userichtexthtml', 'Use rich text html (copy paste from Word, LibreOffice etc)'));    
    ...
    if ($objForm->isFormSubmitted())
        $objHTMLRichText->addText($objHTMLRichTextHidden->getContentsSubmitted(Form::METHOD_POST)->getValue(), false);
    else
    ...
    $objForm->add($objFormatHTML, '', transm($sCurrentModule, 'htmlmarkupcleaner_form_field_formathtml', 'Format HTML code (human readable)'));    
    ...
    $objForm->add($objFilterUnwantedTagsAttributes, '', transm($sCurrentModule, 'htmlmarkupcleaner_form_field_filterunwantedtagsattributes', 'Filter bad HTML markup tags and attributes'));        
    ...
    $objForm->add($objLinksTargetBlank, '', transm($sCurrentModule, 'htmlmarkupcleaner_form_field_linkstargetblank', 'Links: target="_blank"'));        
    ...
    $objForm->add($objLinksRelNofollow, '', transm($sCurrentModule, 'htmlmarkupcleaner_form_field_relnofollow', 'Links: rel="nofollow"'));        
    ...
    $objForm->add($objReplacePByBR, '', transm($sCurrentModule, 'htmlmarkupcleaner_form_field_replacepbybr', 'Replace <p> by <br> tag'));        
    
    
    //submit
    $objSubmit = new InputSubmit();    
    $objSubmit->setValue(transm($sCurrentModule, 'htmlmarkupcleaner_form_button_count', 'Clean my dirty HTML'));
    ...
    if ($objForm->isFormSubmitted())
    {
        saveToFile(explode("\n", $objHTMLRawDirty->getContentsSubmitted(Form::METHOD_POST)->getValue()), $sFileNameLastConversionRawHtml);
        saveToFile(explode("\n", $objHTMLRichTextHidden->getContentsSubmitted(Form::METHOD_POST)->getValue()), $sFileNameLastConversionRichtextHtml);

        $sResult = '';
        if ($objUseRawHtml->getContentsSubmitted(Form::METHOD_POST)->getValue() == 'rawhtml')
            $sResult = $objHTMLRawDirty->getContentsSubmitted(Form::METHOD_POST)->getValue();
        if ($objUseRichtextHtml->getContentsSubmitted(Form::METHOD_POST)->getValue() == 'richtexthtml')
            $sResult = $objHTMLRichTextHidden->getContentsSubmitted(Form::METHOD_POST)->getValue();
        ...
        $objForm->add($objHTMLClean, '', transm($sCurrentModule, 'htmlmarkupcleaner_form_field_html', 'Output: html'));  
        
        
        $objBtnCopyClipboard = new InputButton();
        $objBtnCopyClipboard->setValue(transm($sCurrentModule, 'htmlmarkupcleaner_form_button_copytoclipboard', 'Copy clean HTML to clipboard'));
